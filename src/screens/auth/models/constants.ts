...
export const SIGN_UP_PHONE_SCHEMA = Yup.object().shape({
	...
	location: Yup.string().required(VALIDATION_MESSAGES.required),
	phoneNumber: Yup.string().required(VALIDATION_MESSAGES.required).matches(/^([17][7])[1-9]{6}$/, 'Not a valid number'),
	verificationCode: Yup.string().required(VALIDATION_MESSAGES.required).max(6, 'Verification code is 6 digits long').min(6, 'Verification code is 6 digits long'),
});
