...
const SignUpScreen: FC = () => {
	...
	useEffect(() => {
		if (shouldUpdateProfile) {
			...
			user?.updateProfile({displayName}).catch((error: firebase.FirebaseError) => {console.error(error.message)})
		}
	...
	return (
		<Container style={tailwind('bg-blue-200')}>
			...
			>
				{(formik: FormikProps<{ email: string; password: string, name: string, location: string }>) => (
					<>
						<FMTextInput
							label='Name'
							name='name'
